...
exposure_location_tpms <- snakemake@input[["exposure_location_tpms"]]
...
dds$exposure <- factor(paste(dds$Exposure))
dds$location <- factor(paste(dds$Location))
dds$pc1 <- factor(paste(dds$PC1_sign))
##set reference levels
dds$location <- relevel(dds$location, ref="Dunedin")
dds$treatment <- relevel(dds$exposure, ref="NC")
dds_exposed <- copy(dds)

# run DESeq
design(dds_exposed) <- ~pc1+location+exposure+location:exposure
dds_exposed <- DESeq(dds_exposed)
saveRDS(dds_exposed, snakemake@output[["dds_file"]])

resultsNames(dds_exposed)
res_group <- results(dds_exposed, lfcThreshold = 1, alpha = 0.05)
summary(res_group)
resLFC <- lfcShrink(dds_exposed, coef=5, res=res_group)
...
# read TPMs and merge
exposure_location_tpms <- fread(exposure_location_tpms)
sig_annots_tpm <- merge(sig_annots, exposure_location_tpms, by.x="rn", by.y="#gene_id")
...
##vst transform
#vst <- varianceStabilizingTransformation(dds_exposed, blind=TRUE)
...
#sample_to_label <- data.table(data.frame(colData(dds_exposed)[,c("Location", "Exposure", "sample_name")]))
#sample_to_label <- sample_to_label %>% remove_rownames %>% column_to_rownames(var="sample_name")

#location_colours <- list(Location = c(Dunedin="#fcfdbf", Ruakura="#fc8961"), Experiment=c(Exposed = "#b73779", NC="#51127c"))
