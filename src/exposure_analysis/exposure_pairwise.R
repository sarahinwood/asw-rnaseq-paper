...
exposure_tpms <- snakemake@input[["exposure_tpms"]]
...
# run DESeq
design(dds_exposed) <- ~pc1+location+exposure
...
resLFC <- lfcShrink(dds_exposed, coef=4, res=res_group)
...
# read TPMs and merge
exposure_tpms <- fread(exposure_tpms)
sig_annots_tpm <- merge(sig_annots, exposure_tpms, by.x="rn", by.y="#gene_id")
...
#                         Experiment=c(Exposed = "#b73779", NC="#51127c"))
