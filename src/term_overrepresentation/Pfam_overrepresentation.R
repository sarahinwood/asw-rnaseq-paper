...
pfamannot_table <- fread(term_annot_table_file)
pfamannot_table <- pfamannot_table[,c(1,2)]
pfamannot_table <- distinct(pfamannot_table)

paste("about to run enrichment")
results <- enricher(analysis_DEGs$rn, TERM2GENE=pfam_to_gene, TERM2NAME = pfam_to_name)
