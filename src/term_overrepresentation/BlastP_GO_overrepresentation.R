...
term_annot_table_file <- snakemake@input[["term_annot_table_file"]]

########
# MAIN #
########

analysis_DEGs <- fread(DEG_file)

##GO annot tables
term_to_gene <- fread(term_to_gene_file)
term_to_name<- fread(term_to_name_file)
go_annot_table <- fread(term_annot_table_file)
go_annot_table <- go_annot_table[,c(1,2)]

results <- enricher(analysis_DEGs$rn, TERM2GENE=term_to_gene, TERM2NAME = term_to_name)
##seems legit enough?
results_dt <- data.table(as.data.frame(results), keep.rownames = FALSE)
##merge with annots table
ifelse(length(results_dt$ID>0), paste("sig enrichment"), paste("no sig enrichment"))
