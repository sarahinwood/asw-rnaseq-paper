...
gene_ids <- trinotate_report[!is.na(gene_ontology_BLASTP), unique(`#gene_id`)]
##fairly sure we always used Pfam GO annots for this
go_annot_list<-data.table(trinotate_report[,unique(unlist(strsplit(gene_ontology_BLASTP, "`")))])
...
EXTRACT_GO_TERMS <- function(x, trinotate_report){
  my_terms<-trinotate_report[`#gene_id`==x,unique(unlist(strsplit(gene_ontology_BLASTP, "`")))]
